...
import MapKit
import CoreLocation
//can load map views without core location -- which tracks the current phone
...
//        won't use location until nav manager starts; AccuracyBest is get the most accurate loc but uses more resources (battery, data)
...
//            let restaurant = Restaurant(title: pizzaPlace.name, locationName: pizzaPlace.name, discipline coordinate: loc)
            
            self.map.addAnnotation(pizzaEna)

//            mapItems = array of things we get back
//            pizzaplace could also say marker
//            place = flag where location is going to land
//            1200 is usually a radius
//            setRegion = region around marker
//            name = place
        }
    }
